...
editorialZoomRoot.setAttribute("role", "dialog");
editorialZoomRoot.setAttribute("aria-modal", "true");
editorialZoomRoot.setAttribute("aria-label", "Full-screen image viewer");
...
const openEditorialZoom = (image, trigger = image) => {
  if (!image?.currentSrc && !image?.src) return;

  zoomTrigger = trigger;
  editorialZoomImage.src = image.currentSrc || image.src;
  editorialZoomImage.alt = image.alt || "Full-screen portfolio image";
...
document.querySelectorAll("[data-image-zoom-gallery] .padded-media-item").forEach((item) => {
  const image = item.querySelector("img");
  if (!image) return;

  item.tabIndex = 0;
  item.setAttribute("role", "button");
  item.setAttribute("aria-label", `View ${image.alt || "portfolio image"} full screen`);

  const openImage = () => {
    openEditorialZoom(image, item);
  };
